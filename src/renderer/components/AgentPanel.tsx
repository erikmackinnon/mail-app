...
import { getTimelineFallbackText } from "./agent-timeline-utils";
...
function EventTimeline({ events, runFinished, onAuthRequest, onRetry }: EventTimelineProps) {
  const fallbackText = getTimelineFallbackText(events, runFinished);
  ...
  if (fallbackText && renderedElements.length === 0) {
    renderedElements.push(
      <div
