  ...
  ipcMain.handle(
    ...
    > => {
      ...
      try {
        ...
        const cfg = getConfig();
        const hasAnthropicKey = !!(process.env.ANTHROPIC_API_KEY || cfg.anthropicApiKey);
        const hasOpenAICompatibleConfig = !!cfg.openaiCompatible?.baseUrl;
        const hasLlmConfig =
          (cfg.llmBackend ?? "anthropic") === "openai_compatible"
